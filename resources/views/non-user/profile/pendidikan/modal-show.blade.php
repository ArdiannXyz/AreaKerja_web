                    ...
                    @foreach (Auth::user()->pelamar->riwayat_pendidikan as $pend)
                        <div class="p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between gap-4">
                            <div>
                                ...
                                @if(!empty($pend->asal_pendidikan))
                                    <p class="text-xs font-semibold text-slate-900 mt-0.5">{{ $pend->asal_pendidikan }}</p>
                                @endif
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                <a href="{{ route('pendidikan.edit', $pend->id) }}"
                                    class="p-2 bg-orange-100 hover:bg-orange-200 text-orange-600 rounded-lg transition" title="Edit">
                                ...
                                <form action="{{ route('pendidikan.destroy', $pend->id) }}" method="POST"
                                    onsubmit="return confirm('Yakin hapus pendidikan ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
