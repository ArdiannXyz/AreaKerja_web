{{--
    Komponen: User Badge Dropdown untuk Admin Topbar
    Usage: @include('admin.components.user_badge_dropdown')
--}}
<div class="relative" x-data="{ openUserMenu: false }">
    ...
    <button @click="openUserMenu = !openUserMenu"
        ...
        @else
            <img class="w-8 h-8 rounded-full flex-shrink-0"
                src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->username ?? 'AD') }}&background=00509d&color=fff&size=128"
                alt="Avatar">
        @endif
    ...
    <div x-show="openUserMenu"
        ...
        <div class="px-4 py-3 bg-[#00509d] text-white">
            <p class="text-sm font-bold truncate">{{ Auth::user()?->admin?->nama_lengkap ?: Auth::user()->username }}</p>
