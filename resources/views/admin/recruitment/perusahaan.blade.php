        ...
        <header class="w-full flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
            <div>
                <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
                    <i class="ph ph-briefcase text-[#00509d] text-2xl"></i> Data Perusahaan Recruitment
                </h1>
                <p class="text-xs font-semibold text-slate-500 mt-1">Pilih perusahaan untuk mengelola data recruitment pelamar</p>
            </div>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
            ...
            <div class="relative w-full md:w-80" x-data="{
                ...
                select(val) {
                    this.query = val;
                    this.open = false;
                    $nextTick(() => { $refs.recruitmentPerusahaanForm.submit(); });
                }
            }" @click.outside="open = false">

                <form x-ref="recruitmentPerusahaanForm" action="{{ route('admin.recruitment.perusahaan') }}" method="GET" autocomplete="off" class="flex items-center gap-2 w-full">
                    <div class="h-10 flex items-center w-full bg-white rounded-xl overflow-hidden border border-slate-200 focus-within:border-[#00509d] focus-within:ring-2 focus-within:ring-[#00509d]/20 transition shadow-xs">
                        ...
                        <template x-if="query.length > 0">
                            <a href="{{ route('admin.recruitment.perusahaan') }}"
                               @click.prevent="query = ''; open = false; window.location.href = '{{ route('admin.recruitment.perusahaan') }}';"
                ...
                <table class="w-full text-left border-collapse text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 font-medium text-slate-700">
                        @forelse ($perusahaan as $p)
                            <tr class="hover:bg-blue-50/40 transition {{ $p->user->status == '1' ? 'opacity-60 bg-slate-50' : '' }}">
                                ...
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <a href="{{ route('admin.perusahaan.detail', $p->id) }}"
                                            class="font-bold text-slate-900 hover:text-[#00509d] transition">
                                            {{ $p->nama_perusahaan }}
                                        </a>
                                    </div>
                                </td>
                                ...
                                <td class="px-5 py-3.5 text-center">
                                    <a href="{{ route('admin.recruitment', $p->id) }}"
                                        class="inline-flex items-center gap-1.5 bg-[#00509d] hover:bg-[#003d7a] text-white px-3.5 py-1.5 rounded-xl text-xs font-semibold transition shadow-xs"
                                        title="Buka Data Recruitment">
