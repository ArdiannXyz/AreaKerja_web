...
@section('sidebaradmin')
    <div class="p-4 sm:ml-64" x-data="{ openNotif: false, openAllNotif: false }">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
            ...
            <h1 class="text-xl md:text-2xl font-medium text-center md:text-left">
                Data Recruitment
            </h1>
            ...
            <div class="flex flex-wrap items-center justify-center md:justify-end gap-3">
                ...
                {{-- Profile --}}
                <div
                    class="flex items-center gap-2 bg-white px-3 py-2 border border-gray-500 shadow-md rounded-2xl w-full sm:w-auto">

                    <a href="#">
                        @if (Auth::user()->role == 'admin')
                            @if (Auth::user()->admin->img_profile)
                                <img class="w-10 h-10 object-cover rounded-full"
                                    src="{{ asset('storage/' . Auth::user()->admin->img_profile) }}">
                            @else
                    ...
                    <div class="text-sm w-36 sm:w-auto overflow-hidden">
                        <span class="font-semibold break-all">{{ Auth::user()->username }}</span>
        ...
        <div class="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
            ...
            px-6 py-2 text-md font-medium border-2 rounded-lg transition duration-300">
                    Perusahaan
                </a>
            ...
            <div class="w-full md:w-auto">
                <form action="" method="get" class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari username, nama pelamar, atau nama lowongan..."
                        class="border border-gray-500 rounded-lg px-4 py-2 w-full sm:w-72">

                    <button
                        class="bg-gray-500 hover:bg-gray-400 text-white font-medium px-10 py-2 rounded-xl w-full sm:w-auto">
                        Cari
                    </button>
                </form>
            </div>
        ...
        <div class="overflow-x-auto rounded-2xl border-2 border-gray-400">
            ...
            <table class="w-full text-left border-collapse min-w-[700px] md:min-w-full">
                <thead class="text-center">
                    <tr>
                        <th class="p-7 font-medium min-w-[80px]">ID</th>
                        <th class="p-7 font-medium min-w-[150px]">Nama Kandidat</th>
                        <th class="p-7 font-medium min-w-[150px]">Lowongan</th>
                        <th class="p-7 font-medium min-w-[180px]">Email</th>
                        <th class="p-7 font-medium min-w-[130px]">Telepon</th>
                        <th class="p-7 font-medium min-w-[100px]">Aksi</th>
                    </tr>
                </thead>
                ...
                <tbody class="text-center">
                    @forelse ($recruitments as $r)
                        <tr class="border-b-[2px] border-gray-300 hover:bg-gray-100">
                            ...
                            <td class="px-4 py-3 break-words whitespace-normal">{{ $r->id }}</td>

                            <td class="px-4 py-3 break-words whitespace-normal">
                                {{ $r->pelamar->nama_pelamar }}
                            </td>
