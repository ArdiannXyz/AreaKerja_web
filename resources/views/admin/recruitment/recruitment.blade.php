        ...
        <header class="w-full flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.recruitment.perusahaan') }}"
                   ...
                   title="Kembali ke Perusahaan Recruitment">
                    <i class="ph ph-arrow-left text-base"></i>
                </a>
                <div>
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
                        <i class="ph ph-users text-[#00509d] text-2xl"></i> Recruitment - {{ $perusahaan->nama_perusahaan }}
                    </h1>
                    <p class="text-xs font-semibold text-slate-500 mt-1">Daftar kandidat yang telah direkrut oleh perusahaan ini</p>
                </div>
            </div>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
            ...
            <div class="relative w-full md:w-80" x-data="{
                ...
                recommendations: [
                    ...
                    { label: 'Developer', category: 'Posisi', icon: 'ph-code' },
                    { label: 'Desainer', category: 'Posisi', icon: 'ph-paint-brush' },
                    { label: 'Staff', category: 'Posisi', icon: 'ph-briefcase' },
                ],
                get filtered() {
                ...
                <form x-ref="recruitmentSearchForm" action="{{ route('admin.recruitment', $perusahaan->id) }}" method="GET" autocomplete="off" class="flex items-center gap-2 w-full">
                    <div class="h-10 flex items-center w-full bg-white rounded-xl overflow-hidden border border-slate-200 focus-within:border-[#00509d] focus-within:ring-2 focus-within:ring-[#00509d]/20 transition shadow-xs">
                        ...
                        <template x-if="query.length > 0">
                            <a href="{{ route('admin.recruitment', $perusahaan->id) }}"
                               @click.prevent="query = ''; open = false; window.location.href = '{{ route('admin.recruitment', $perusahaan->id) }}';"
                ...
                <table class="w-full text-left border-collapse text-sm">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider">
                            ...
                            <th class="px-5 py-4">Nama Kandidat</th>
                            <th class="px-5 py-4">Lowongan Terkait</th>
                    ...
                    <tbody class="divide-y divide-slate-100 font-medium text-slate-700">
                        @forelse ($recruitments as $r)
                            <tr class="hover:bg-blue-50/40 transition">
                                <td class="px-5 py-3.5 text-center font-bold text-slate-500">{{ $r->id }}</td>

                                <td class="px-5 py-3.5 font-bold text-slate-900">
                                    <div class="flex items-center gap-3">
                                        @if ($r->pelamar->img_profile)
                                            <img src="{{ asset('storage/' . $r->pelamar->img_profile) }}" alt="Avatar" class="w-9 h-9 rounded-xl object-cover border border-slate-200">
                                        @else
                                            <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-[#00509d] to-[#0077b6] text-white font-bold flex items-center justify-center text-xs flex-shrink-0 shadow-xs">
                                                {{ strtoupper(substr($r->pelamar->nama_pelamar ?? ($r->pelamar->user->username ?? 'P'), 0, 1)) }}
                                            </div>
                                        @endif
                                        <span>{{ $r->pelamar->nama_pelamar ?? ($r->pelamar->user->username ?? '-') }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-5 py-3.5">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-50 text-[#00509d] border border-blue-200/60">
                                        {{ $r->lowonganPerusahaan->nama ?? '-' }}
                                    </span>
                                </td>

                                <td class="px-5 py-3.5 text-slate-600 text-xs">
                                    {{ $r->pelamar->user->email ?? '-' }}
                                </td>

                                <td class="px-5 py-3.5 text-slate-600 text-xs">
                                    {{ $r->pelamar->telepon_pelamar ?? '-' }}
                                </td>

                                <td class="px-5 py-3.5 text-center">
                                    <a href="{{ route('admin.recruitment.detail', $r->id) }}"
                                        ...
                                        title="Lihat Detail Recruitment">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-12 text-center text-slate-400">
                                    ...
                                    Tidak ada data recruitment untuk perusahaan ini.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
