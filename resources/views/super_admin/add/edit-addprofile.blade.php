...
<main class="flex-1 p-4 sm:p-6 sm:ml-64 bg-slate-50/70 min-h-screen" x-data="{ openNotif: false, openAllNotif: false }">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-sm">
        <div>
            ...
            <nav class="flex items-center gap-2 text-xs text-slate-400 mb-1.5">
                ...
                <a href="{{ route('superadmin.detail.user', $user->id) }}" class="hover:text-[#00509d] transition font-medium">
                    Detail User
                </a>
                <i class="ph ph-caret-right text-[10px]"></i>
                <span class="text-slate-600 font-semibold">Edit User</span>
            </nav>
            <h1 class="text-lg sm:text-xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
                Edit Profil Pengguna: <span class="text-[#00509d]">{{ $user->username }}</span>
            </h1>
        </div>
    ...
    <div class="max-w-4xl mx-auto bg-white border border-slate-100 rounded-2xl p-6 sm:p-8 shadow-sm w-full overflow-x-hidden">
            ...
            <form action="{{ route('superadmin.update.user', $user->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-100 mt-8">
                    <a href="{{ route('superadmin.detail.user', $user->id) }}"
                        class="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 font-semibold text-xs transition duration-150">
                        Batal
                    </a>
                    <button type="submit"
                        class="inline-flex items-center gap-2 bg-[#00509d] hover:bg-[#003d7a] text-white px-6 py-2.5 rounded-xl font-semibold text-xs shadow-sm hover:shadow transition duration-150">
