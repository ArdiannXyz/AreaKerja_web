...
@section('sidebarsuperadmin')
    <main class="flex-1 p-4 sm:p-6 sm:ml-64 bg-[#f8fafc] min-h-screen overflow-y-auto" x-data="{ openNotif: false, openAllNotif: false }">
        ...
        @php
            ...
            $kategoriKey = strtolower($data->kategori ?? 'kandidat aktif');
            $kategoriParam = $mapKategori[$kategoriKey] ?? 'kandidat';
            ...
            $primaryAlamat = $data->alamat_pelamar->first();
        @endphp

        {{-- TOP NAVIGATION & ADMIN USER BAR (Standard Superadmin Header) --}}
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 pb-4 border-b border-slate-200">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-slate-800 tracking-tight whitespace-nowrap">Detail Kandidat</h1>
                    <p class="text-xs text-slate-500">Informasi profil lengkap dan kurikulum vitae kandidat</p>
                </div>
            </div>

            {{-- Superadmin Notif & User Badge Dropdown --}}
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
                ...
                <div class="flex items-center gap-4">
                    <div class="relative flex-shrink-0">
                        ...
                        @else
                            <img class="w-16 h-16 sm:w-20 sm:h-20 object-cover rounded-2xl border-2 border-slate-100 shadow-sm"
                                src="https://ui-avatars.com/api/?name={{ urlencode($data->nama_pelamar ?? 'Kandidat') }}&background=00509d&color=fff&size=128"
                                alt="{{ $data->nama_pelamar }}">
                        @endif

                        @if ($kategoriKey === 'kandidat aktif')
                            <div class="absolute -bottom-1 -right-1 bg-emerald-500 text-white p-1 rounded-full border-2 border-white shadow-sm" title="Kandidat Aktif">
                    ...
                    <div>
                        <div class="flex items-center gap-3 flex-wrap">
                            <h2 class="text-lg sm:text-xl font-bold text-slate-900">
                                {{ $data->nama_pelamar ?: ($data->user->username ?? 'Kandidat') }}
                            </h2>
                            
                            {{-- Status Badge (Clean & Never wraps) --}}
                            @if ($kategoriKey === 'kandidat aktif')
                                <span class="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200/80 whitespace-nowrap">
                                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                                    Kandidat Aktif
                                </span>
                            @elseif ($kategoriKey === 'calon kandidat')
                            ...
                            @else
                                <span class="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200/80 whitespace-nowrap">
                                    Pelamar
                                </span>
                            @endif
                        </div>
                        ...
                        <div class="flex items-center gap-2.5 text-xs text-slate-500 mt-1 flex-wrap">
                            ...
                            @if (!empty($data->divisi))
                                <span>•</span>
                                <span class="text-blue-600 font-semibold">{{ is_array($data->divisi) ? implode(', ', $data->divisi) : $data->divisi }}</span>
                            @endif
                        </div>
                    </div>
                </div>

                {{-- Right: Candidate Action Buttons (Cleanly aligned in a dedicated row) --}}
                <div class="flex items-center gap-2.5 flex-wrap pt-2 lg:pt-0 border-t lg:border-t-0 border-slate-100">
                    ...
                    {{-- Edit Data --}}
                    <a href="{{ route('superadmin.pelamar.edit', ['kategori' => $kategoriParam, 'id' => $data->id]) }}"
                        class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 text-xs font-semibold transition shadow-sm whitespace-nowrap">
                    ...
                    <form action="{{ route('superadmin.pelamar.destroy', $data->id) }}" method="POST"
                        onsubmit="return confirm('Apakah Anda yakin ingin menghapus data kandidat {{ $data->nama_pelamar }}? Tindakan ini tidak dapat dibatalkan.');"
                        class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                    ...
                    <div class="grid grid-cols-3 gap-2 text-center">
                        ...
                        <div class="bg-slate-50/80 rounded-xl p-2.5 border border-slate-100">
                            ...
                            <span class="text-xs font-semibold text-blue-600 mt-0.5 block truncate capitalize">
                                {{ $data->kategori ?: 'Kandidat' }}
                            </span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-3.5 text-xs">
                        ...
                        <div class="flex items-start gap-3 p-2.5 rounded-xl bg-slate-50/60 border border-slate-100">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <a href="mailto:{{ $data->user->email }}" class="text-xs font-semibold text-slate-800 hover:text-blue-600 truncate block transition" title="{{ $data->user->email }}">
                                    {{ $data->user->email }}
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-3 p-2.5 rounded-xl bg-slate-50/60 border border-slate-100">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <span class="text-xs font-semibold text-slate-800 block">
                                    {{ $data->tanggal_lahir ? $data->tanggal_lahir->translatedFormat('d F Y') : 'Belum diisi' }}
                                </span>
                            </div>
                        </div>
                        ...
                        {{-- Periode Pelatihan --}}
                        @if ($data->mulai_pelatihan || $data->selesai_pelatihan)
                            <div class="flex items-start gap-3 p-2.5 rounded-xl bg-blue-50/50 border border-blue-100">
                                <div class="p-2 rounded-lg bg-white border border-blue-200 text-blue-600">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                    </svg>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <span class="text-[11px] font-medium text-blue-600 block">Masa Pelatihan</span>
                                    <span class="text-xs font-semibold text-slate-800 block">
                                        {{ $data->mulai_pelatihan ? $data->mulai_pelatihan->format('d M Y') : '-' }} s/d {{ $data->selesai_pelatihan ? $data->selesai_pelatihan->format('d M Y') : '-' }}
                                    </span>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="flex items-center justify-between p-2.5 rounded-xl bg-slate-50/60 border border-slate-100 hover:bg-slate-50 transition">
                            ...
                            @if (!empty($data->sosmed->instagram))
                                <a href="{{ $formatUrl($data->sosmed->instagram, 'instagram') }}" target="_blank"
                                    class="text-blue-600 hover:text-blue-700 font-semibold truncate max-w-[140px] text-right flex items-center gap-1">
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                    <div class="flex items-center gap-3 pb-4 mb-4 border-b border-slate-100">
                        ...
                        <div>
                            <h3 class="text-base font-bold text-slate-900">Tentang Kandidat</h3>
                    ...
                    <div class="text-sm text-slate-600 leading-relaxed bg-slate-50/50 rounded-xl p-4 border border-slate-100">
                        ...
                        @else
                            <p class="text-slate-400 italic text-xs">Kandidat belum mengisi deskripsi diri.</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                    <div class="flex items-center justify-between pb-4 mb-4 border-b border-slate-100">
                        ...
                        <span class="text-xs font-semibold px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg">
                            {{ count($data->skill) }} Keahlian
                        </span>
                    </div>

                    <div class="flex flex-wrap gap-2">
                        @forelse ($data->skill as $s)
                            <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-blue-50/70 border border-blue-200/70 text-blue-800 text-xs font-medium">
                                ...
                                @if (!empty($s->experience_level))
                                    <span class="text-[10px] font-normal px-1.5 py-0.5 rounded bg-blue-200/60 text-blue-700">
                                        {{ $s->experience_level }}
                                    </span>
                                @endif
                            </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                    <div class="flex items-center justify-between pb-4 mb-5 border-b border-slate-100">
                        ...
                        <span class="text-xs font-semibold px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg">
                            {{ $data->pengalaman_kerja->count() }} Riwayat
                        </span>
                    </div>

                    @if ($data->pengalaman_kerja->isNotEmpty())
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                    <div class="flex items-center justify-between pb-4 mb-5 border-b border-slate-100">
                        ...
                        <span class="text-xs font-semibold px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg">
                            {{ $data->riwayat_pendidikan->count() }} Pendidikan
                        </span>
                    </div>

                    @if ($data->riwayat_pendidikan->isNotEmpty())
                        <div class="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                    <div class="flex items-center justify-between pb-4 mb-5 border-b border-slate-100">
                        ...
                        <span class="text-xs font-semibold px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg">
                            {{ $data->pengalaman_organisasi->count() }} Organisasi
                        </span>
                    </div>

                    @if ($data->pengalaman_organisasi->isNotEmpty())
                        <div class="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
                ...
                <div class="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 rounded-2xl p-6 text-white shadow-md flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="flex items-center gap-4 text-center sm:text-left">
                        ...
                        <div>
                            <h4 class="text-base font-bold">Curriculum Vitae (CV) Kandidat</h4>
