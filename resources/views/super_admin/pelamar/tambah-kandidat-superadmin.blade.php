...
@section('sidebarsuperadmin')
    <main class="flex-1 p-4 sm:p-6 sm:ml-64 bg-slate-50/70 min-h-screen" x-data="{ openNotif: false, openAllNotif: false }">
        ...
        @php
            ...
            $kategori = $kategori ?? request()->route('kategori') ?? 'kandidat';
        @endphp
        ...
        <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-xs text-slate-400">Pelamar / <span class="text-slate-500 font-medium">Tambah Data</span></p>
        ...
        <div class="max-w-5xl mx-auto space-y-6">
            ...
            {{-- Alerts --}}
            @if (!$pelamarSudahAda)
                <div class="bg-blue-50 border border-blue-200 text-blue-800 p-4 rounded-2xl flex items-start gap-3 shadow-sm">
                    <div class="p-1 bg-blue-100 text-blue-700 rounded-lg flex-shrink-0 mt-0.5">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    <div class="text-xs leading-relaxed">
                        <span class="font-bold block text-sm mb-0.5 text-blue-900">Langkah Pengisian Data</span>
                        Harap isi dan simpan <strong>Informasi Akun & Data Diri Utama</strong> terlebih dahulu. Setelah disimpan, Anda dapat melengkapi Alamat, Pendidikan, Organisasi, Pengalaman Kerja, dan Keahlian.
                    </div>
                </div>
            @endif
            ...
            {{-- MAIN FORM --}}
            <form action="{{ route('superadmin.pelamar.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf

                {{-- Hidden Kategori Input --}}
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
                    <div class="flex items-center gap-3 pb-5 mb-6 border-b border-slate-100">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">Kredensial login akun dan identitas dasar kandidat</p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-8 p-4 rounded-2xl bg-slate-50/70 border border-slate-100">
                        ...
                        <div class="space-y-2 text-center sm:text-left">
                            ...
                            <p class="text-[11px] text-slate-500">Format yang didukung: JPG, PNG, atau WebP. Ukuran maksimal 2MB.</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs">
                        ...
                        <div>
                            ...
                            <input type="text" name="username" value="{{ old('username', $pelamar?->user?->username) }}"
                                ...
                                placeholder="Contoh: sitirahayu" required />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="email" name="email" value="{{ old('email', $pelamar?->user?->email) }}"
                                class="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition"
                        ...
                        <div>
                            <label class="block font-semibold text-slate-700 mb-1.5">
                                Kata Sandi <span class="text-rose-500">{{ $pelamarSudahAda ? '' : '*' }}</span>
                            </label>
                            <input type="password" name="password"
                                class="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition"
                                placeholder="{{ $pelamarSudahAda ? 'Kosongkan jika tidak ingin diubah' : 'Buat kata sandi akun' }}"
                                {{ $pelamarSudahAda ? '' : 'required' }} />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_lahir"
                                value="{{ old('tanggal_lahir', $pelamar && $pelamar->tanggal_lahir ? $pelamar->tanggal_lahir->format('Y-m-d') : '') }}"
                                class="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition" />
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="deskripsi_diri" rows="3"
                                class="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition leading-relaxed"
                                placeholder="Tuliskan ringkasan pengalaman, keahlian utama, atau motivasi kandidat...">{{ old('deskripsi_diri', $pelamar->deskripsi_diri ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
                    <div class="flex items-center gap-3 pb-5 mb-6 border-b border-slate-100">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">Alamat domisili, pendidikan, rekam karir, dan keahlian</p>
                        </div>
                    </div>
                        ...
                        <div class="p-4 rounded-2xl border border-slate-200/80 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    @if ($pelamarSudahAda && $pelamar->alamat_pelamar->count() > 0)
                            ...
                            <div class="flex items-center gap-2 self-end sm:self-auto">
                                ...
                                <button type="button"
                                    ...
                                    @if (!$pelamarSudahAda) onclick="alert('Harap simpan data utama kandidat terlebih dahulu sebelum menambahkan Alamat.')"
                        ...
                        <div class="p-4 rounded-2xl border border-slate-200/80 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    @if ($pelamarSudahAda && $pelamar->riwayat_pendidikan->count() > 0)
                                        <p class="text-[11px] text-slate-500 mt-0.5">
                            ...
                            <div class="flex items-center gap-2 self-end sm:self-auto">
                                ...
                                <button type="button"
                                    class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold transition shadow-sm"
                                    @if (!$pelamarSudahAda) onclick="alert('Harap simpan data utama kandidat terlebih dahulu sebelum menambahkan Pendidikan.')"
                        ...
                        <div class="p-4 rounded-2xl border border-slate-200/80 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    @if ($pelamarSudahAda && $pelamar->pengalaman_kerja->count() > 0)
                                        <p class="text-[11px] text-slate-500 mt-0.5">
                            ...
                            <div class="flex items-center gap-2 self-end sm:self-auto">
                                ...
                                <button type="button"
                                    class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold transition shadow-sm"
                                    @if (!$pelamarSudahAda) onclick="alert('Harap simpan data utama kandidat terlebih dahulu sebelum menambahkan Pengalaman Kerja.')"
                        ...
                        <div class="p-4 rounded-2xl border border-slate-200/80 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    @if ($pelamarSudahAda && $pelamar->pengalaman_organisasi->count() > 0)
                                        <p class="text-[11px] text-slate-500 mt-0.5">
                            ...
                            <div class="flex items-center gap-2 self-end sm:self-auto">
                                ...
                                <button type="button"
                                    class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold transition shadow-sm"
                                    @if (!$pelamarSudahAda) onclick="alert('Harap simpan data utama kandidat terlebih dahulu sebelum menambahkan Organisasi.')"
                        ...
                        <div class="p-4 rounded-2xl border border-slate-200/80 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    @if ($pelamarSudahAda && $pelamar->skill->count() > 0)
                                        <p class="text-[11px] text-slate-500 mt-0.5">
                            ...
                            <div class="flex items-center gap-2 self-end sm:self-auto">
                                ...
                                <button type="button"
                                    class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold transition shadow-sm"
                                    @if (!$pelamarSudahAda) onclick="alert('Harap simpan data utama kandidat terlebih dahulu sebelum menambahkan Keahlian.')"
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
                    ...
                    @php
                        $disabled = !session('pelamar_terakhir_id') && !$pelamarSudahAda ? 'disabled' : '';
                    @endphp
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                        ...
                        <div>
                            ...
                            <input type="text" name="social_media[instagram]"
                                ...
                                class="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition disabled:bg-slate-50 disabled:text-slate-400"
                                placeholder="Username atau link Instagram" {{ $disabled }} />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="social_media[linkedin]"
                                ...
                                class="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition disabled:bg-slate-50 disabled:text-slate-400"
                                placeholder="Link profil LinkedIn" {{ $disabled }} />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="social_media[website]"
                                ...
                                class="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition disabled:bg-slate-50 disabled:text-slate-400"
                                placeholder="https://portofolio-anda.com" {{ $disabled }} />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="social_media[twitter]"
                                ...
                                class="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition disabled:bg-slate-50 disabled:text-slate-400"
                                placeholder="Username atau link Twitter/X" {{ $disabled }} />
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit"
                        ...
                        <span>Simpan Data Kandidat</span>
                    </button>
                </div>
