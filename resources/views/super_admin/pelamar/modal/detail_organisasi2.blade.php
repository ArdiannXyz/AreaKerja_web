<!-- Main modal -->
<div id="show-org2" tabindex="-1" aria-hidden="true"
                   ...
                   @foreach ($pelamar->pengalaman_organisasi as $org)
                    <div class="mb-6 mt-4">
                        <div class="flex justify-between ml-4">
                            ...
                            <a href="{{ route('superadmin.organisasi.edit', $org->id) }}">
                        ...
                        <div class="flex gap-2 mt-2 ml-4">
                            <!-- Tombol Hapus -->
                            <form action="{{ route('superadmin.organisasi.destroy', $org->id) }}" method="POST"
                                onsubmit="return confirm('Yakin hapus organisasi ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="px-3 py-1 bg-red-500 text-white rounded-lg text-sm hover:bg-red-600 font-medium">Hapus</button>
                            </form>
                        </div>
                    </div>
                @endforeach
